...
def evaluate(args, accelerator, model, eval_dataset, eval_dataloader):
    ...
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            ...
            start_pred, end_pred = post_process_function_greedy(pred_logits, context_length)
...
def train(args, accelerator, model, train_dataset, train_dataloader, optimizer, lr_scheduler, eval_dataset, eval_dataloader, tokenizer):
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.output_dir is not None:
            logger.info(f'best {best_F1} current {F1}')
            if args.save_best:
                if best_F1 < F1:
                    best_F1 = F1
...
def main():
    ...
    config.update({'num_iterations':args.num_iterations})
    config.update({'temp':args.temp})
    config.update({'alpha':args.alpha})
    ...
    if args.do_predict:
        logger.info("\n\n\n")
        logger.info("***** Running Test Start *****")
        model = BertForFrameSRL.from_pretrained(
            args.output_dir,
            from_tf=bool(".ckpt" in args.model_name_or_path),
            config=config,
