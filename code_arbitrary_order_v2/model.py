...
def mfvi(unary_scores, x, num_iterations, temp=1.0, alpha=1.0):
    """
    Args:
    ...
    Returns:
        q: tensor of shape (num_roles, num_states), updated role probabilities
    """

    # num_roles, num_states, num_rank = x.shape
    #
    # q = unary_scores.softmax(dim=-1) # Initialize q with softmax of unary_scores
    #
    ...
    #         q[i] = (unary_scores[i]+sum_over_rank).softmax(dim=-1)
    #
    # return q

    num_roles, num_states, num_rank = x.shape

    q = (unary_scores/temp).softmax(dim=-1)  # Initialize q with unary_scores
    ...
    for t in range(num_iterations):
        ...
        q = (((unary_scores+sum_over_rank)/temp).softmax(dim=-1)-q)*alpha+q
...
class BertForFrameSRL(BertPreTrainedModel):
    ...
    def forward(
        ...
        for i in range(input_ids.shape[0]):
            ...
            flattened_pred_logit=mfvi(flattened_unary_logit, flattened_cpd_logit, self.num_iterations, self.temp, self.alpha)
            pred_logit=torch.full((FE_num, cont_len, cont_len), 1e-36).to(sequence_output.device)
            ...
            total_loss += self.criterion(torch.log(pred_logit+1e-36),gold_position_indices)
